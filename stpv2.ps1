...
Begin {
    ...
    Function Step-SetVariable {
        ...
        $value = $setupVariable.GetEnumerator() | 
                 Where-Object {$_.name -eq $variable -and $_.value} | 
    ...
    Function Step-saveInput {
        ...
        If (Test-Path -Path $iniFile -ErrorAction SilentlyContinue) {
            Remove-Item -Path $iniFile -Force -Confirm:$false
        }
    ...
    Function Step-ChooseFile {
        ...
        If ($FileChoice -eq [Windows.Forms.DialogResult]::OK) {
    ...
    Function Step-exportCred {
        Param (
            [Parameter(Mandatory = $true,HelpMessage='Provide Credentials')][System.Management.Automation.Credential()][pscredential]$credential,
        ...
        Begin {
            ...
            [Void][IO.Directory]::CreateDirectory((Split-Path -Path $xmlPath -Parent))
        }
        Process {
...
Process {
    ...
    Function Step-OnScreenMSG {
        ...
        Switch ($z) {
            1 {
                ...
                Write-Host 'Fast and safe test of the features, this solution offers.'
                Write-Host '        - Run this installer on a test computer (preferably VM [Windows Sandbox, Virtualbox, Hyper-V, etc.])'
                Write-Host "        - No prerequisities needed like '" -NoNewLine
                Write-Host 'Active Directory' -NoNewLine -ForegroundColor Cyan
                Write-Host "' or a '" -NoNewLine
                Write-Host 'Cloud Repository' -NoNewLine -ForegroundColor Cyan
                Write-Host "'."
                Write-Host '    - GOAL    : ' -NoNewLine -ForegroundColor Yellow
                Write-Host 'To have this as simple as possible - Installer automatically:'
                Write-Host '        - Installs VSC, GIT.'
                Write-Host '        - Creates GIT repository in ' -NoNewLine
                Write-Host $remoteRepository -NoNewLine -ForegroundColor Cyan
                Write-Host '. (and clone it to ' -NoNewLine
                Write-Host $userRepository -NoNewLine -ForegroundColor Cyan
                Write-Host '.)'
                Write-Host '        - Creates folder ' -NoNewLine
                Write-Host $repositoryShareLocPath -NoNewLine -ForegroundColor Cyan
                Write-Host ' and shares it as ' -NoNewLine
                Write-Host $repositoryShare -NoNewLine -ForegroundColor Cyan
                Write-Host '.'
                ...
                Write-Host ' Standard installation (Active Directory needed)' -ForegroundColor Cyan
                ...
                Write-Host "        - Creates a GPO '" -NoNewLine
                Write-Host $GPOname -NoNewLine -ForegroundColor Cyan
                Write-Host "' that will be used for connecting clients to this solution:"
                ...
                Write-Host ' Update of existing installation' -ForegroundColor Cyan
    ...
    If (!$noEnvModification) {
        ...
        $choice = ''
        Do {
            $choice = Read-Host -Prompt 'Choose install option (1|2|3)'
    ...
    Try {
                    ...
                    $invokeParam.ScriptBlock  = {
                        ...
                        If ($repositoryShareLocPath) {
                            ...
                            If (Test-Path -Path $repositoryShareLocPath) {
                                ...
                                [Void][IO.Directory]::CreateDirectory($repositoryShareLocPath)
                                ...
                                [Void][IO.Directory]::CreateDirectory($subfolder)
                                # share the folder
        ...
        If (!$testInstallation) {
            ...
            [Void][IO.Directory]::CreateDirectory($remoteRepository)
        ...
        If ($testInstallation -or (!$noEnvModification -and !(Step-skip))) {
            ...
            [Void][IO.Directory]::CreateDirectory($profileSnippets)
